...
export default function middleware(req: NextRequest) {
  ...
  // Get hostname of request (e.g. demo.tennis.so)
  ...
  if (hostname === "tennis.so" || hostname === "platforms.vercel.app") {
    url.pathname = "https://demo.tennis.so";
    return NextResponse.redirect(url);
  }

  const currentHost =
    process.env.NODE_ENV === "production" && process.env.VERCEL === "1"
      ? // You have to replace ".tennis.so" with your own domain if you deploy this example under your domain.
